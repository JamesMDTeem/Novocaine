 ...
 * kept separate so the marker overlay and LpExplorer (LP-discovery tracking) can both depend on
 * this instead of on each other. Ported from nurgling2's HarvestState.
 */
public class HarvestState {
     ...
     * (e.g. "granite3"), but LpSpec.object tracks them by the bare species name - strips that
     * suffix so lookups against those maps work regardless of which variant a gob is currently
     * showing. A no-op for every other resource.
     */
     ...
     * live state data the 3D renderer itself uses - not a per-resource-type guess. Throws Loading
    ...
    // already have the decoded sdt in hand (the tree/bush specs, which decode it once to check
    // both) don't need to re-derive the bit meanings themselves.
     ...
     * seed/fruit product - derived directly from LpSpec.object (a species with a seasonal pair
    ...
    // resolving a non-tree/bush product's icon via LpSpec.getIconPath()) that don't have a tree
    ...
    /**
     * Stands in for "fell this tree to reach its wood products" on a standing tree whose log's
     * Board/Block are still undiscovered (see LpExplorer.derivedResource). Unlike getIcon()'s
     * categories this isn't a per-species product icon at all - there's no such item - so it's a
     * single fixed tool icon rather than anything keyed by species.
     */
    public static BufferedImage chopIcon() {
        return loadIcon("gfx/invobjs/stoneaxe");
    }
     ...
     * bitmask bit set, or a tracked resource with no LpSpec entry) but couldn't resolve an actual
     * icon for - almost always a species new to the game that hasn't been added to the data maps
     * yet. Makes that gap visible in-game instead of silently showing nothing, so it gets noticed
     * and reported rather than going unnoticed indefinitely.
     */
