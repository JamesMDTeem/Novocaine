...
/**
 * Per-gob-type configuration for the always-visible harvest-icon overlay: which resources it
 * applies to, how its icons are laid out, which setting turns it on, and what icon(s) it
 * currently has to show. Two shapes of gob exist today - see TreeHarvestSpec/BushHarvestSpec
 * (fixed category slots read from a live per-instance bitmask) and ProductListHarvestSpec (a
 * flat LpSpec.object product list, always "present") - both implement this same interface so the
 * overlay doesn't need to know which kind of gob it's showing.
 */
public interface HarvestSpec {
    ...
    LpConfig.Key masterToggle();

    /**
     * The current set of icons to show for this gob, each flagged if its product is still
     * LP-undiscovered (so the overlay can tint just that one). May throw haven.Loading if the
    ...
    class Part {
        ...
        // product name) - used to build the overlay's per-gob label cache key, since the same
