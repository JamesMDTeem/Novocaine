...
public class MenuGrid extends Widget implements KeyBinding.Bindable {
	...
	public List<ItemInfo> info() {
		...
		if(ItemInfo.ResourceName.enabled())
	...
	public BufferedImage rendertt(boolean withpg) {
	    ...
	    if(withpg) {
		/* Copy first: info() hands back the list it caches, so removing the
		 * name in place would strip it from every later reader as well. */
	...
	private void loadCustomActionButtons() {
		...
		// Category: Novocaine (resource path still says NurglingImports - see gen-menugrid-res.py)
		makeLocal("customclient/menugrid/NurglingImports/NBotsSettings");
		makeLocal("customclient/menugrid/NurglingImports/NBotPlaces");
		makeLocal("customclient/menugrid/NurglingImports/LpAssistant/AutoLpBot");
		makeLocal("customclient/menugrid/NurglingImports/LpAssistant/LpAssistantManager");
		makeLocal("customclient/menugrid/NurglingImports/LpAssistant/LpAssistantToggle");
		...
		makeLocal("customclient/menugrid/NurglingImports/CrewBots/NSmelterBot");
	...
	public void useCustom(String[] ad) {
		...
		if (ad[1].equals("Toggles")) { // Category: Toggles
			...
			} else if (ad[2].equals("AlchemyHelper")) {
				/* Same pattern again. Turning it off also drops everything the container pool
				 * remembered - see AlchemyHelperWindow.tick. */
