...
public class GItem extends AWidget implements ItemInfo.SpriteOwner, GSprite.Owner, RandomSource {
	...
	public default Pipe.Op rstate() {
	    /* olcol() can throw Loading, and this op gets applied deep inside
	     * the draw where nothing catches it. Resolve the colour here instead -
	     * rstate() is called from AttrCache, which absorbs Loading and retries
	     * next frame, so a half-loaded icon just draws untinted for a frame. */
	    Color col = olcol();
	    if(col == null)
		return(buf -> {});
